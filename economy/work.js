...
const MoneyTimeDb = require("../moneySever");
const moneyTime = new MoneyTimeDb();

exports.run = async (client, message, args, config) => {
  let amount = Math.floor(Math.random() * 3000) + 1;
  let workType = Math.floor(Math.random() * 357);
  let timeout = 21600000;
  let workTime;
  let checkAccount = message.author.id;
  await moneyTime.findFinal(checkAccount).then((times) => {
    if (times !== undefined) {
      workTime = times;
    } else {
      workTime = 0;
  ...
  if (workTime !== null && timeout - (Date.now() - workTime) > 0) {
    ...
    return message.channel.send(
      userMention +
        ` You already worked, you can come back and work in **${time.hours}h ${time.minutes}m ${time.seconds}s**!`
    );
  } else {
    let whatWork = possibleWork[workType];

    let embed = new Discord.MessageEmbed()
      .setAuthor(
        `${message.author.tag}, it payed off!`,
        message.author.displayAvatarURL
      )
      .setDescription(
        `${message.author}, you've worked as **${whatWork}** and earned $${amount}!`
      )
      .setColor("GREEN");

    message.channel.send(embed);
    await Money.appendXp(message.author.id, message.guild.id, amount);
    await moneyTime.set(checkAccount);
  }
};
